...
const LevelController = {
    ...
    getLevelForLanguage: async (req,res)=>{
        ...
        try{
            db.query(`select * from levels where language_id=?`,
                [lang_id],
                (err,result)=>{
                    if(err) return err;
                    else{
    ...
    //     let lang_url=req.params.lang_url;
    //     let lang_id;
    //     try{

    //         db.query(`select id from languages where url=?`,[lang_url],
    //             (err,result)=>{
    //                 if(err) return err;
    //                 else{
    //                     console.log(result)
    //                     lang_id=result[0]?.id;
    //                     db.query(`select * from levels where language_id=?`,[lang_id],
    //                         (err,result)=>{
    //                             if(err) return err;
    //                             else{
    //                                 res.status(200).send(result)
