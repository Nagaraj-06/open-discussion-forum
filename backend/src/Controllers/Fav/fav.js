...
const Fav_Controllers = {
    ...
    Getfavourites: async (req,res)=>{
        ...
        try{
            ...
            let list=[]
            db.query("select * from likes where email=? and status=?",[email,'1'],
              (err,result)=>{
                if(err){
                    console.log(err);
                }
    ...
    ReplyFavourites: async (req,res)=>{
        ...
        try{
            let list=[]
            const {email}=req.body;
            db.query("select * from reply_likes where email=? and status=?",[email,'1'],
                (err,result)=>{
                    if(err){
                        console.log(err);
                    }else{
    ...
    try{
        ...
        db.query("select email,status from likes where post_id=? and email=?",[id,email],(err,result)=>{
            if(err) {
                console.log(err);
            }
            else{              
    ...
    addlike: async (req,res)=>{
        ...
        try{
            ...
            db.query("select status from likes where email=? and post_id=?",
                [email,id],
                (err,result)=>{
                    if(err) {
                        console.log(err);
                        ...
                        if(result.length==0) {
                            db.query("insert into likes(email,post_id,status) values (?,?,?)",
                                [email,id,status],
                                    (err,result)=>{
                                        if(err) {
                                            console.log(err);
                                ...
                                db.query("update likes set status=? where email=? and post_id=?",
                                    [status,email,id],
                                        (err,result)=>{
                                            if(err) {
                                                console.log(err);
    ...
    add_reply_like: async (req,res)=>{
        ...
        try{
            ...
            if(SubReplyId==null) {
                sqlquery="select status from reply_likes where email=? and post_id=? and MainReplyId=?";
                queryParams=[email,id,MainReplyId]
            }else if(MainReplyId==null) {
                sqlquery="select status from reply_likes where email=? and post_id=? and SubReplyId=?";
            ...
            db.query(sqlquery,queryParams,
                (err,result)=>{
                    if(err) {
                        ...
                        if(result.length==0) {
                            if(SubReplyId==null) {
                                sqlquery="insert into reply_likes(email,post_id,MainReplyId,status) values (?,?,?,?)";
                                queryParams=[email,id,MainReplyId,status]
                            }else if(MainReplyId==null) {
                                sqlquery="insert into reply_likes(email,post_id,SubReplyId,status) values (?,?,?,?)";
                            ...
                            db.query(sqlquery,
                                ...
                                if(SubReplyId==null) {
                                    sqlquery="update reply_likes set status=? where email=? and post_id=? and MainReplyId=?";
                                    queryParams=[status,email,id,MainReplyId]
                                }else if(MainReplyId==null) {
                                    sqlquery="update reply_likes set status=? where email=? and post_id=? and SubReplyId=?";
    ...
    post_likes: async (req,res)=>{
        ...
        try{
            ...
            db.query("select * from likes where post_id=? and status=?",[id,'1'],
                (err,result)=>{
                    if(err) {
                        console.log(err);
                    }else{
    ...
    //         const status = 0;
    //         db.query("update likes set status = where email=? and post_id=? ",[email,id,],
    //             (err,result) =>{
    //                 if(err) {
    //                     console.log(err);
    //                 }else{
