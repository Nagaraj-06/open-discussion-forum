...
const PostsControllers = {
    ...
    try{
        ...
        if(image) {
            sqlquery ='INSERT INTO questions (email,language_id,level_id,title,body,date,time,image) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
            queryParams=  [email,language_id,level_id,title,body,date,time,image];
        }
        else{
            sqlquery ='INSERT INTO questions (email,language_id,level_id,title,body,date,time) VALUES (? ,? ,?, ?, ?, ?, ?)';
    ...
    EditDiscussion: async (req,res)=>{
        ...
        try{
            ...
            if(image) {
                sqlquery ='update questions set language_id=?,level_id=?,title=?,body=?,image=? where id=?';
                ...
                if(pre_image!=null) {
                    ...
                    sqlquery ='update questions set language_id=?,level_id=?,title=?,body=?,image=? where id=?';
                    queryParams=  [language_id,level_id,title,body,pre_image,edit_id] ;
                ...
                db.query(
                    ...
                    (err, result) => {
                        if (err) {
                            ...
                            db.query("update reply_details set language_id=? ,level_id=? where post_id=?",[language_id,level_id,edit_id],
                            (err,result)=>{
    ...
    getposts: async (req,res)=>{
        try{
            ...
            db.query('select * from questions where language_id=?',
            [language_id],
            (err,result)=>{
                if(err){
                    console.log(err);
    ...
    Getpost_programLevel : async (req,res)=>{
        ...
        db.query('SELECT * FROM questions WHERE language_id = ? AND level_id = ?', [language_id, level_id], (err, result) => {
            if (err) {
                console.log(err);
                res.status(500).send(err);
            } else {
