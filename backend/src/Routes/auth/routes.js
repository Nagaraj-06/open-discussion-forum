...
const VerifyUser= (req,res,next) =>{
    ...
    if(!token) {
        ...
        if(user == "token expired") {
            ...
            db.query('update users set date=?,time=? where email=?',[fecha,fecha1, req.email ],(err,resultt)=>{
                if(err) {
                    console.log(err);
    ...
    db.query("select email from users where email=? ",[userEmail],(err,result)=>{
        if(err) {
            ...
            if(result.length>0) {   
               ...
               db.query('update users set date=?,time=? where email=?',[fecha,fecha1,userEmail],(err,resultt)=>{
                    if(err) {
                        console.log(err);
                        return res.status(500).send('Server error');
...
router.get("/logout", (req, res) => {
    ...
    db.query('update users set date=?,time=? where email=?',[fecha,fecha1, email ],(err,resultt)=>{
        if(err) {
            console.log(err);
            return res.status(500).send('Server error');
